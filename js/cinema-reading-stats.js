class ReadingStats {
  ...
  getReadingHistory() {
    return JSON.parse(localStorage.getItem('historicoAssistidos') || '{}');
  }

  // CORRIGIDO: Calcula os dias de sessão com base no histórico real
  getReadingDays() {
    const history = this.getReadingHistory();
    // Conta o número de dias únicos que têm registro de sessão
    return Object.keys(history).length;
  }

  // CORRIGIDO: Calcula o total de minutos lidas com base no histórico
  getTotalPagesRead() {
    const history = this.getReadingHistory();
  ...
  updateGoalProgress() {
    ...
    if (goalCard) {
      if (booksRead >= this.goalTarget) {
        ...
        goalCard.title = 'Meta de sessão completada!';
      } else {
  ...
  getBooksReadCount() {
    ...
    return filmesAssistidos.length;
  }
  ...
  getGenreStats() {
    ...
    filmesAssistidos.forEach(filme => {
      if (filme.generos) {
        const genres = Array.isArray(filme.generos) ? filme.generos : filme.generos.split(',').map(g => g.trim());
  ...
  getAuthorStats() {
    ...
    const authorCount = {};
    filmesAssistidos.forEach(filme => {
      if (filme.diretor) authorCount[filme.diretor] = (authorCount[filme.diretor] || 0) + 1;
    });
  ...
  generateGitHubChart() {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      dayElement.title = `${dateFormatted}: ${pagesOnDate} minutos lidas`;
  ...
  renderAuthorsChart() {
    ...
    if (authorStats.length === 0) {
      chartContainer.innerHTML = '<p style="color: rgba(255,255,255,0.7); text-align: center;">Nenhum diretor registrado ainda</p>';
    }
  }
  ...
  // Método para ser chamado quando um filme é adicionado/removido
  refresh() {
